/*
 * 3D Gallery view 1.1 - jQuery plugin
 *
...
(function($){
	...
	$.fn.dddgallery = function(options) {
		...
		$(j_obj).children().hide();
		$(j_obj).append('<div id="loading"><img src="image/loader.gif"></div>');
		if(!window_loaded) {
		...
		$( ".box" ).mouseenter(function() {
				if(!$(this).hasClass('box_principale')){
					if(!isdrag){
						...
						$(this).css(cssObj_Ent).append('<div class="vedi">阡陌</div>'); //aggiungo il pulsante per vederlo in primo piano
						//quando clicco vedi
		...
		$( ".box" ).dblclick( function(){ iv = $(this).attr("alt"); vediclick(iv); })
		...
		function buildtreddi() {
			...
			for(i=1; i<=n_box; i++){
				//verifico se l'oggetto in questione è più alto che largo
				if($(j_obj).children("div:nth-child("+i+")").height()>=$(j_obj).children("div:nth-child("+i+")").width()){
					piualto = true;
				}else{
					piualto = false;
				}
				...
				if($(j_obj).children("div:nth-child("+i+")").hasClass('box_principale')){
					...
					if(piualto){
						cooh [i] = opts.p_height; //larghezza
						coow [i] = ($(j_obj).children("div:nth-child("+i+")").width()*opts.p_height )/$(j_obj).children("div:nth-child("+i+")").height();
					}else{
						coow [i] = opts.p_width; //larghezza
						cooh [i] = ($(j_obj).children("div:nth-child("+i+")").height()*opts.p_width )/$(j_obj).children("div:nth-child("+i+")").width();
					}
				...
				//imposto una dimensione in base alla profondità
					if(piualto){
						cooh [i] = opts.min_height + cool [i] * ((opts.max_height-opts.min_height)/(opts.livel_range-1));					
						coow [i] = ($(j_obj).children("div:nth-child("+i+")").width()*cooh [i] )/$(j_obj).children("div:nth-child("+i+")").height();
					}else{
				...
				//imposto la posizione radom [le finestre non devono essere celate dal box_principale e neanche devono essere fuori la finestra
					...
					if((coox [i] > ((global_x / 2 )-(opts.p_width / 2)-(coow [i]/2)))&&(coox [i] < ((global_x / 2 )+(opts.p_width / 2)-(coow [i]/2)))){
						...
						if (upordown == 1){
							cooy [i] = Math.round(Math.random()*((global_y / 2 )-(opts.p_height / 2)-(cooh [i]/2)));
						}else{
							cooy [i] = ((global_y / 2 )+(opts.p_height / 2)-(cooh [i]/2))+Math.round(Math.random()*((global_y-(cooh [i]/2))-((global_y / 2 )+(opts.p_height / 2)-(cooh [i]/2))));
						}
					}else{
...
/*					$.each(cssObj_B, function(index, value) { 
				...
				$(".box img").css({'width' : '100%', 'height' : '100%'});
		}
		...
		function vediclick(iv){
			...
			if(i_p){
				...
				//imposto una dimensione in base alla profondità
					if($(j_obj).children("div:nth-child("+i_p+")").height()>=$(j_obj).children("div:nth-child("+i_p+")").width()){
						cooh [i_p] = opts.min_height + cool [i_p] * ((opts.max_height-opts.min_height)/(opts.livel_range-1));					
						coow [i_p] = ($(j_obj).children("div:nth-child("+i_p+")").width()*cooh [i_p] )/$(j_obj).children("div:nth-child("+i_p+")").height();
					}else{
						coow [i_p] = opts.min_width + cool [i_p] * ((opts.max_width-opts.min_width)/(opts.livel_range-1));					
						cooh [i_p] = ($(j_obj).children("div:nth-child("+i_p+")").height()*coow [i_p] )/$(j_obj).children("div:nth-child("+i_p+")").width();
					}
				...
				//imposto la posizione radom [le finestre non devono essere celate dal box_principale e neanche devono essere fuori la finestra
					...
					if((coox [i_p] > ((global_x / 2 )-(opts.p_width / 2)-(coow [i_p]/2)))&&(coox [i_p] < ((global_x / 2 )+(opts.p_width / 2)-(coow [i_p]/2)))){
						upordown = Math.round(Math.random());
						if (upordown == 1){
							cooy [i_p] = Math.round(Math.random()*((global_y / 2 )-(opts.p_height / 2)-(cooh [i_p]/2)));
						}else{
							cooy [i_p] = ((global_y / 2 )+(opts.p_height / 2)-(cooh [i_p]/2))+Math.round(Math.random()*((global_y - (cooh [i_p]/2))-((global_y / 2 )+(opts.p_height/ 2)-(cooh [i_p]/2))));
						}
					}else{
			...
			//calcolo le nuove dimensioni del principale
			if($(j_obj).children("div:nth-child("+iv+")").height()>=$(j_obj).children("div:nth-child("+iv+")").width()){
				cooh [iv] = opts.p_height; //larghezza
				coow [iv] = ($(j_obj).children("div:nth-child("+iv+")").width()*opts.p_height )/$(j_obj).children("div:nth-child("+iv+")").height();

			}else{
				coow [iv] = opts.p_width; //larghezza
				cooh [iv] = ($(j_obj).children("div:nth-child("+iv+")").height()*opts.p_width )/$(j_obj).children("div:nth-child("+iv+")").width();
			}

			cool [iv] = opts.livel_range+2; //livello
			coox [iv] = (global_x / 2 ) - (coow [iv] / 2); //posizione x
